...
class GoogleMapState extends gmap.GoogleMapStateBase {
  ...
  void _updateMarkersRoute(Set<tryThis.Marker> markers) {
    markers.forEach((element) {
      var func = element.onTap!;
      addMarkerRaw(
        GeoCoord(element.position.latitude, element.position.longitude),
            ...
            if (status == DirectionsStatus.OK) {
              ...
              waypoints?.forEach((element) {
                if (_markers_colection["chargerPoints"]!.containsKey(element.toString())) {
                  ...
                  addMarker(items_t.Marker(element, icon: "packages/google_maps_cluster_manager/assets/images/defaultMarkerWeb.png",), group: "route");
                }
              });
    ...
    if (_map == null) {
      ui.platformViewRegistry.registerViewFactory(htmlId, (int viewId) {
        ...
        if (_map != null) {
          ...
          _manager_route.setMapZoomW(cur_zoom);
        }
